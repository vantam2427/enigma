...
class AbstractPage(unittest.TestCase, Config):
    ...
    def openBrowser(self, driver, profile = "normal"):
        try: 
            if AbstractPage.Browser() == None:
                if driver == "Firefox":
                    if profile == "normal":
                        profilePath = str(getCurentSrcPath() + "../../../../../resources/zfh90odn.default").replace("/","\\")
                        ...
                        prof.set_preference("browser.startup.homepage_override.mstone","ignore") # Remove first time page
                        prof.set_preference("startup.homepage_welcome_url.additional",  "about:blank") # Remove first time page
                        AbstractPage.browser = webdriver.Firefox(firefox_profile=prof)
                    ...
                    AbstractPage.browser = webdriver.Ie(str(getCurentSrcPath() + "../../../../../resources/IEDriverServer.exe").replace("/","\\"), capabilities = caps)
        except Exception, e:
            print str(e) 
    ...
    # @author vinh.cong.tran
    ##############################################################################################################         
    def navigateToWeb(self, url, driver, disableBronto = True):
        try:
    ...
    def navigateToWebWithGetUpToSpeed(self, url, driver):
        try:
            ...
            logInfo("Navigate to %s successfully with the Get Up To Speed popup" % url)
            return AbstractPage.Browser()
        except Exception, e:
            print str(e)
            logWarning("Navigate to %s unsuccessfully" % url)
...
#             logInfo(str(e))
    ...
    def getWindowSize(self):
        try:
            return AbstractPage.Browser().get_window_size()
        except Exception, e:
            logInfo(str(e))
    ...
    def getPositionOfElement(self, element_xPath):
        try:
            ...
            logInfo(str(e))
    ##############################################################################################################
    # Navigate to a url and handle the Lightbox Bronto Subscribe Form popup and switch Language
    # @param url: Url path, language
    # @param driver: Type of browser
    # @return browser object
    # @author
    ##############################################################################################################        
    def navigateToWebAndLanguage(self, url, language, disableBronto = True):
        if language == 'US':
            url = url
        elif language == 'CA':
            url = url.replace('en-us', 'en-ca')
        elif language == 'UK':
            url = url.replace('en-us', 'en-gb')
        elif language == 'DE':
            url = url.replace('en-us', 'de-de')
        elif language == 'AT':
            url = url.replace('en-us', 'de-at')
        elif language == 'CH':
            url = url.replace('en-us', 'de-ch')
        elif language == 'UAE':
            url = url.replace('en-us', 'en-ae')
        self.navigateToWeb(url, self.cfDriver, disableBronto)      